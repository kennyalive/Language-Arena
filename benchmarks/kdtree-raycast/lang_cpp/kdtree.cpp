...
KdTree::KdTree(std::vector<Node>&& nodes, std::vector<int32_t>&& triangleIndices, const TriangleMesh& mesh,
    const BoundingBox_f& meshBounds)
    : _nodes(std::move(nodes))
    , _triangleIndices(std::move(triangleIndices))
    , _mesh(mesh)
...
, _meshBounds(mesh.getBounds())
{
    ...
    file.read(reinterpret_cast<char*>(triangleIndices.data()), triangleIndicesCount * 4);
    if (!file)
    ...
    while (closestIntersection.t > tMin)
    {
        if (node->isInteriorNode())
        {
            ...
            else
            {
                ...
                double tSplit = distanceToSplitPlane * ray.getInvDirection()[axis]; // != 0
        ...
        else // leaf node
        {
            ...
            else
            {
                ...
                {
                    auto indices = _mesh.triangles[_triangleIndices[node->index + i]].vertexIndices;
                    Triangle triangle;
                    triangle.p[0] = Vector(_mesh.vertices[indices[0]]);
                    triangle.p[1] = Vector(_mesh.vertices[indices[1]]);
                    triangle.p[2] = Vector(_mesh.vertices[indices[2]]);
...
const BoundingBox_f& KdTree::getMeshBounds() const
{
    return _meshBounds;
}
...
void KdTree::saveToFile(const std::string& fileName) const
{
    ...
    file.write(reinterpret_cast<const char*>(_nodes.data()), _nodes.size() * sizeof(Node));
    ...
    file.write(reinterpret_cast<const char*>(_triangleIndices.data()), _triangleIndices.size() * 4);
}
