...
struct TBoundingBox
{
    ...
    RayIntersection intersect(const Ray& ray) const
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            double tNear = (minPoint[i] - ray.getOrigin()[i]) * ray.getInvDirection()[i];
            double tFar = (maxPoint[i] - ray.getOrigin()[i]) * ray.getInvDirection()[i];
    ...
    static TBoundingBox<T> boundsUnion(const TBoundingBox<T>& bounds, const TBoundingBox<T>& bounds2)
    {
        return TBoundingBox<T>
        (
            TVector<T>
            (
                std::min(bounds.minPoint.x, bounds2.minPoint.x),
                std::min(bounds.minPoint.y, bounds2.minPoint.y),
                std::min(bounds.minPoint.z, bounds2.minPoint.z)
            ),
            TVector<T>
            (
                std::max(bounds.maxPoint.x, bounds2.maxPoint.x),
