...
enum
{
    // max count is chosen such that maxTrianglesCount * 2 is still an int32_t, this simplifies implementation.
...
KdTreeBuilder::KdTreeBuilder(const TriangleMesh& mesh, const BuildParams& buildParams)
: _mesh(mesh)
...
, _buildStats(buildParams.collectStats)
{
    ...
    {
        throw KdTreeBuildingException("Exceeded the maximum number of mesh triangles: " +
            std::to_string(maxTrianglesCount));
    }

    if (_buildParams.maxDepth <= 0)
        _buildParams.maxDepth = std::lround(8.0 + 1.3 * std::floor(std::log2(mesh.getTrianglesCount())));

    _buildParams.maxDepth = std::min(_buildParams.maxDepth, static_cast<int>(KdTree::maxTraversalDepth));
}
...
KdTree KdTreeBuilder::buildTree()
{
    ...
    _trianglesBuffer.resize(_mesh.getTrianglesCount() * (_buildParams.maxDepth + 1));
    ...
    buildNode(meshBounds, _trianglesBuffer.data(), _mesh.getTrianglesCount(), _buildParams.maxDepth,
        _trianglesBuffer.data(), _trianglesBuffer.data() + _mesh.getTrianglesCount());

    _buildStats.finalizeStats();

    KdTree tree;
    tree._nodes = std::move(_nodes);
    tree._triangleIndices = std::move(_triangleIndices);
    return tree;
}

void KdTreeBuilder::buildNode(const BoundingBox_f& nodeBounds, const int32_t* nodeTriangles, int32_t nodeTrianglesCount,
    int depth, int32_t* triangles0, int32_t* triangles1)
{
    if (_nodes.size() >= KdTree::Node::maxNodesCount)
        throw KdTreeBuildingException("The maximum number of KdTree nodes has been reached: " +
    ...
    if (nodeTrianglesCount <= _buildParams.leafCandidateTrianglesCount || depth == 0)
    {
    ...
    for (int32_t i = split.edge + 1; i < 2 * nodeTrianglesCount; ++i)
    {
        ...
        {
            int32_t triangle = _edgesBuffer[i].triangleAndEndFlag & BoundEdge::triangleMask;
...
void KdTreeBuilder::createLeaf(const int32_t* nodeTriangles, int32_t nodeTrianglesCount)
{
    KdTree::Node node;
    ...
    {
        node.initLeafWithMultipleTriangles(nodeTrianglesCount, static_cast<int32_t>(_triangleIndices.size()));
        _triangleIndices.insert(_triangleIndices.end(), nodeTriangles, nodeTriangles + nodeTrianglesCount);
    }
}

KdTreeBuilder::Split KdTreeBuilder::selectSplit(const BoundingBox_f& nodeBounds, const int32_t* nodeTriangles,
...
KdTreeBuilder::Split KdTreeBuilder::selectSplitForAxis(const BoundingBox_f& nodeBounds, int32_t nodeTrianglesCount,
    int axis) const
{
    ...
    const float invTotalS = 1.0f / (2.0f * (diag.x*diag.y + diag.x*diag.z + diag.y*diag.z));
    const int32_t numEdges = 2 * nodeTrianglesCount;

    Split bestSplit = { -1, axis, _buildParams.intersectionCost * nodeTrianglesCount };
    ...
    while (i < numEdges)
    {
        ...
        int groupEnd = i + 1;
        while (groupEnd < numEdges && edge.positionOnAxis == _edgesBuffer[groupEnd].positionOnAxis)
            ++groupEnd;
        ...
        if (t > nodeBounds.minPoint[axis] && t < nodeBounds.maxPoint[axis])
        {
            ...
            auto emptyBonus = (numBelow == 0 || numAbove == 0) ? _buildParams.emptyBonus : 0.0f;

            auto cost = _buildParams.traversalCost +
                (1.0f - emptyBonus) * _buildParams.intersectionCost * (pBelow*numBelow + pAbove*numAbove);
...
void KdTreeBuilder::BuildStats::finalizeStats()
{
    ...
    trianglesPerLeaf = static_cast<double>(_trianglesPerLeafAccumulated) / notEmptyLeafCount;
