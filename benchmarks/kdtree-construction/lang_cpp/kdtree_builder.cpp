      ...
      _buildStats(buildParams.collectStats)
{
    ...
    _buildParams.maxDepth = std::min(
        _buildParams.maxDepth, static_cast<int>(KdTree::maxTraversalDepth));
}
                              ...
                              int32_t* triangles0, int32_t* triangles1)
{
    ...
    _nodes[thisNodeIndex].initInteriorNode(split.axis, aboveChild,
                                           splitPosition);
                                                ...
                                                int32_t nodeTrianglesCount)
{
    ...
    for (int index = 0; index < 3; ++index) {
        ...
        if (split.edge != -1) {
            if (_buildParams.splitAlongTheLongestAxis) return split;
            if (split.cost < bestSplit.cost) bestSplit = split;
        }
    }
    ...
    while (i < numEdges) {
        ...
        if (t > nodeBounds.minPoint[axis] && t < nodeBounds.maxPoint[axis]) {
            ...
            if (cost < bestSplit.cost) {
                bestSplit.edge =
                    (middleEdge == groupEnd) ? middleEdge - 1 : middleEdge;
                bestSplit.cost = cost;
            }
        }
...
void KdTreeBuilder::BuildStats::finalizeStats()
{
    ...
    averageDepth =
        static_cast<double>(_leafDepthAccumulated) / notEmptyLeafCount;
