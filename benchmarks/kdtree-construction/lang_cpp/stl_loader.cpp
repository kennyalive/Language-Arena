...
TriangleMeshLoadingException::TriangleMeshLoadingException(
...
{
  enum { headerSize = 84, facetSize = 50 };
  ...
  if (fileSize == std::streampos(-1))
    RuntimeError("failed to read file: " + fileName);
  ...
  if (std::memcmp(fileContent.data(), asciiStlHeader.data(), 5) == 0)
    throw TriangleMeshLoadingException("Ascii stl files are not supported: " +
                                       fileName);

  int32_t numTriangles =
      *reinterpret_cast<int32_t*>(fileContent.data() + headerSize - 4);

  auto expectedSize =
      headerSize + static_cast<size_t>(numTriangles) * facetSize;
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      if (iterator == uniqueVertices.cend()) {
        if (mesh->vertices.size() >
          ...
          throw TriangleMeshLoadingException(
              "Too large model: too many vertices");
